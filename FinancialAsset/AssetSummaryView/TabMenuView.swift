                    ...
                    VStack(spacing: 0) {
                        HStack {
                            ...
                            Text(tabs[row])
                                ...
                                .foregroundColor(tabIndex == row ? .accentColor : .gray)
                                .offset(x: -4, y: 0)
                        }
                        .frame(height: 52)
                        
                        Rectangle()
                            .fill(tabIndex == row ? Color.secondary : Color.clear)
                            .frame(height: 3)
                            .offset(x: 4, y: 0)
                    }
                })
...
struct TabMenuView_Previews: PreviewProvider {
    static var previews: some View {
        TabMenuView(tabs: ["지난달 결제", "이번달 결제", "다음달 결제"], tabIndex: .constant(1), updated: .constant(.currentMonth(amount: "10,000원")))
    }
}
